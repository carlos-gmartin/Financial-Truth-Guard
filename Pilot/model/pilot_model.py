...
def read_data():
    ...
    true_file_path = '../data/finance_dataset/Finance_TRUE.csv'
    fake_file_path = '../data/finance_dataset/Finance_FAKE.csv'
...
def preprocess_text(df):
    print("X length:", len(df['text']))
    print("Y length:", len(df['label']))
...
def train_model(X_train, y_train, X_test, y_test, vocab_size, max_len):
    ...
    checkpoint_path = 'model_checkpoint_pilot_5.h5'
...
if __name__ == "__main__":
    ...
    model_file = 'cnn_model_pilot.h5'
    tokenizer_file = 'tokenizer_pilot.pkl'
    ...
    if not (os.path.exists(model_file) and os.path.exists(tokenizer_file)):
        ...
        model.save('cnn_model_pilot.h5')
        joblib.dump(tokenizer, 'tokenizer_pilot.pkl')
